  ...
  .post(async (req, res) => {
    ...
    try {
      ...
      if (postData.roleInput === "user") {
        ...
        if (
          typeof postData.preferGenreInput !== "string" &&
          !Array.isArray(postData.preferGenreInput)
        ) {
      ...
      if (
        postData.emailAddressInput.match(
          /[a-zA-Z0-9]+([_.-][a-zA-Z0-9]+)*@[a-zA-Z0-9-]+[.]([a-zA-Z][a-zA-Z][a-zA-Z]*)/
        ) !== null
      )
        match = postData.emailAddressInput.match(
          /[a-zA-Z0-9]+([_.-][a-zA-Z0-9]+)*@[a-zA-Z0-9-]+[.]([a-zA-Z][a-zA-Z][a-zA-Z]*)/
        )[0];
    ...
    try {
      const success = await authDataFuncs.registerUser(
        postData.emailAddressInput,
        postData.passwordInput,
        postData.ageInput,
        postData.roleInput,
        postData.preferGenreInput
      );
...
router
  ...
  .post(async (req, res) => {
    ...
    try {
      const user = await authDataFuncs.loginUser(
        postData.emailAddressInput,
        postData.passwordInput
      );
      if (user) {
